...
export function registerTools(server: McpServer, db: Database): void {
  ...
  server.registerTool(
    'recall_query',
    {
      ...
      description: [
        ...
        '',
        'ccRecall is a project-scoped long-tail memory store. It complements',
        'the curated MEMORY.md index that loads at session start: MEMORY.md',
        'carries a small hand-picked set; ccRecall carries the rest.',
        '',
        'USE THIS WHEN:',
  ...
  server.registerTool(
    'recall_save',
    {
      ...
      description: [
        'Save durable project knowledge to ccRecall — decisions, discoveries,',
        'preferences, recurring patterns, or corrections worth recalling in',
        'future sessions.',
        '',
        'This is the highest-quality signal in the knowledge base. Auto-harvest',
        'captures session metadata; manual saves capture insight. Use both: the',
        'curated MEMORY.md index for the small hand-picked set, manual recall_save',
        'for the long tail of project knowledge that should outlive a session.',
        '',
        'USE WHEN the user or assistant identifies:',
        '- A decision with rationale that future sessions should know',
        '- A non-obvious finding (debugging breakthrough, gotcha, root cause)',
        '- A user preference or convention worth applying broadly',
        '- A recurring pattern or workflow template',
        '- A correction on past work',
        '',
        'Prefer concise self-contained facts over generic session summaries.',
