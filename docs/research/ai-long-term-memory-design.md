...
- **Topic**: 超越 /save-t 的 AI 長期記憶機制設計，核心問題：元認知（AI 知道自己知道什麼）
...
這份文件來自子超和 Claude 在 2026-04-13 的一次深度討論。起因是研究 Obsidian 分享會教學包時，延伸到 AI 記憶長久化的根本問題。子超明確指出：這是目前 AI 體系最重大的問題之一。
...
5. 提出記憶分離架構（身份/反應/知識三層）→ 子超認為值得深挖
6. 分析 /save-t 的痛點 → 子超指出問題不只是 /save-t，而是 Claude Code 記憶機制的根本限制
7. 討論收斂到核心問題：**元認知——AI 不知道自己知道什麼**
8. 子超的觀察：「AI 一定不會承認自己不知道」「關掉 session 就遺忘一切」
...
子超的判斷：情節記憶「某種程度上可以靠某些機制來滿足」（hooks + session 摘要），但元認知是更根本的難題。
...
子超的實際痛點：之前用 AI-Handshake.md + Serena MCP 做記憶，累積大量 markdown 文件，每次讀取浪費大量 context window。/save-t、AI-Handshake、obsidian-mind 的 SessionStart 都會撞上這面牆——差別只是撞牆的速度。
...
4. **取代 /save-t**：新系統設計出來後，/save-t 的哪些功能保留、哪些被新機制取代？
5. **AI 的根本限制**：「AI 不會承認自己不知道」——只能靠 hooks 強制？還是有其他路？
6. **子超的歷史方案**：AI-Handshake.md + Serena MCP 的具體實作方式值得回顧，作為新方案的 baseline
...
- 子超的歷史方案：AI-Handshake.md + Serena MCP（待回顧）
