...
ccRecall silently injected that memory into Claude's context as a ~400-token note (the default budget; callers can raise it via `maxTokens` up to 2000), and he picked up where you left off.
