...
> 用 **pnpm** 裝？第一次用 global 需要先跑一次 `pnpm setup`（建 global bin 目錄、改 shell rc 加 `PNPM_HOME` 到 PATH），重開 terminal 後再 `pnpm add -g @tznthou/ccrecall`。Yarn v1 類似，`yarn global add` 前確認 `yarn global bin` 在 PATH 上。
