...
- **抽取沒產出東西時會自動重跑一次。** 0.7.x 加的靜默漏抽偵測看得到失敗，卻什麼
  也沒做——模型把 `recall_save(...)` 印成文字的那種跑法，記憶就一直留在資料庫外面，
  直到有人剛好看了終端機一眼。拿其中一次失敗**用同一個模型**重跑，第一次嘗試就抽
  出四筆，這才是重試值得花的理由：那不是模型能力的天花板，只是從來沒有人重跑過。
  只重試一次，不做迴圈——第二次還失敗是該浮上來的訊號，不是該繼續付錢的事。

### 變更

- **抽取改用 Sonnet 5，不再用 Haiku 4.5**，正式 wrapper 與 backfill 救援腳本兩處
  都改。理由是記憶品質，不是可靠度。

  一筆記憶只有前 149 字進得了注入，所以開頭在敘述問題的記憶送到時是不能用的——
  `extraction-prompt.md` 自己把「處方前置」標為最重要的一條。用 11 份真實 transcript、
  86 筆記憶做盲審，判準在任何資料產生之前就寫死、來源在所有判定寫完之後才揭曉：
  **Haiku 56% 處方前置，Sonnet 84%**。改用逐份 transcript 的配對比較——也就是把
  「某份 transcript 本來就比較難」這個因素控制掉——兩邊都有產出的 9 份裡 Sonnet 贏 7 份，
  平均 **+32 個百分點**（Wilcoxon p = 0.0273；把整批漏抽算 0% 則 p = 0.0068）。

  可靠度**不是**理由，也不拿它當理由：Haiku 有 2/10 份 transcript 完全沒產出、Sonnet 是 0，
  但這個樣本數下差異並不顯著（Fisher p = 0.47），而且上面那個重試機制本來就抓得到那兩次。

  代價是 2.9 倍的時間（11 份 transcript 合計 927 秒 → 2712 秒）與 2 倍的單價。抽取是在
  對話結束後背景跑的，沒有人在等它。

  有一件事值得誠實寫下來：第一批 4 份 transcript 量到的是 44% 對 90%，第二批獨立的 6 份
  量到 65% 對 79%——後者單獨看在這個樣本數下並不顯著（p = 0.34）。第一批是運氣好的抽樣。
  這個決定靠的是合併與配對的結果，不是第一批那張漂亮的表。

### 修復

- **我們一直用一個大約八次只看得到一次的偵測器在量失敗率。** 既有的 marker 只認得
  一種失敗長相：模型把呼叫語法印出來。另一種——模型安靜地什麼都沒做——在遙測裡
  跟「這場對話本來就沒東西好存」是 byte-identical，而這句話程式碼自己的註解已經
  寫了好幾個月。

  拿 257 次乾淨抽取去跟資料庫對帳才有數字：19 次零寫入，其中 12 次是 transcript
  太短或已被清掉、零本來就是正確答案，但**有 7 次握著 52–200KB 的實質內容卻一筆
  都沒存**，marker 一個都沒抓到。現在抽取會去查這次到底寫進幾筆 `agent-inferred`
  記憶，實質 transcript 卻零寫入就照它的真面目報成可疑漏抽。

  有一條計數規則值得寫下來，因為搞錯它正是這件事被藏住的原因：使用者在對話中手動
  存的記憶是 `explicit`，**不算**抽取的產出。把它們算進去，壞掉的那次看起來就是成功的。

  資料庫一律唯讀開啟，而且測試直接斷言「透過這個 handle 寫入會失敗」，不是信任意圖
  ——這是 wrapper 第一次碰資料庫，而 ccRecall 的契約是唯讀。查詢根本跑不起來時
  （沒有 `sqlite3`、沒有資料庫、資料庫被鎖），結果是「未知」而不是零：沉默不等於
  失敗的證據，把它當證據會讓沒裝 `sqlite3` 的機器每次都重跑。

  遙測列多了 `retried`、`transcriptBytes`、`memoriesWritten`，下一個要量這件事的人
  就不必再從資料庫回推一次。
...
- **抽取出來的記憶可以指出自己來自哪一則訊息** — `memories.message_id` 1,639 筆
  全是空的，而卡住的從來不是 schema：欄位與 `recall_save` 參數本來就都在。真正的
  原因是抽取用的 transcript 每一輪都標成 `--- human ---`，長得一模一樣，模型根本
  無從指認。現在標頭帶上訊息 uuid（`--- human [<uuid>] ---`），prompt 要模型回報
  那筆記憶來自哪一則訊息的 uuid，寫入端再拿它跟該 session 真正有的訊息核對——對不
  上的直接丟掉，記憶照存。來源資訊只增不減，不會反過來擋住內容。**但存下來的出處
  並不代表那則訊息證明了這筆記憶。** 兩邊的文字沒有任何比對，核的只有「這個 uuid
  存不存在」和「屬不屬於這個 session」，prompt 裡對模型也是這樣講的。它換到的是
  「掰出來的 uuid 進不去」，而這正是這個欄位值得填的全部理由——一個裝得下捏造值的
  出處欄位比空的更糟，因為它看起來像證據。代價是 transcript 每則訊息多約 40 bytes
  （實測：151 則訊息的 session 增加 4.93%），已經頂到 200KB 上限的 session 會從尾
  端少送這麼多。
...
- **「記憶到底有沒有幫上忙」的三臂比較跑分器**（`evaluation/`，`pnpm eval:demo`）
  —— 目前所有指標量的都是檢索層：正確的那筆有沒有回來、有沒有送進注入、有沒有被
  重複浮出。沒有一個回答得了
  [#71](https://github.com/tznthou/ccRecall/issues/71)。這支跑分器把同一個問題
  放進三種證據條件跑：真正的檢索路徑、一個刻意寫笨的詞袋基準、以及完全沒有記憶，
  然後比較結果。**基準那一臂才是關鍵**：贏過「沒有記憶」只證明有 context 比沒有
  好，贏過一個 15 行的排序器才撐得起我們這套排序機制。答題的 callback 只收到
  `{ question, evidence }`，分不出自己在哪一臂；每個案例把語料載入專屬的資料庫，
  絕不開啟正式庫；另有一個案例的語料根本答不出它自己的問題，讓整套有個底線——
  指標一旦開始量的是模型自身知識就會被它抓到。demo 走離線 stub 模型，不需要
  API key、零成本，也不宣稱任何答案品質。**首次結果（n = 5，stub 模型）：笨基準
  領先，0.600 對 0.400。** 兩個已知弱點解釋了它：改述、以及沒有空格的中文問句。
  五個手寫案例定不了任何結論，但三臂確實分得開、底線也穩定停在零。
### 修復

- **`~/.ccrecall` 跟裡面的東西現在只有你自己讀得到** —— 這個目錄裝著資料庫（全部
  記憶、全部 session 標題），還有每一列都帶 `cwd` 的遙測記錄——那是你的帳號名稱，
  以及你做過的每一個專案名。建立這個目錄的地方有四處，只有一處給了 mode，所以誰先
  跑到誰說了算。實際上先跑到的是 session-start hook：每開一次 session 就跑一次，
  而它沒給 mode——預設安裝下留下 `0755`，在好幾個發行版預設的 `umask 002` 下則是
  `0775`，同機器上的其他使用者寫得進去。資料庫檔案本身是 `0644`：全世界可讀，在
  dogfood 機器上那是 106MB 的記憶。

  四個建立者現在一律把目錄開成 `0700`，資料庫、遙測記錄與 integrity alert 則是
  `0600`。**既有安裝也會被修好，不是只有新裝的** —— `mkdirSync` 不會動既有目錄的
  mode，寫檔時的 `mode` 也只在建檔當下生效，所以少了明確的修復動作，這個修法對任何
  已經裝了 ccRecall 的人等於沒發生。修復刻意只限 `~/.ccrecall` 底下的路徑：PR #94
  試過對可設定的路徑重設 mode，結果把使用者自己的專案目錄從 `0755` 改成 `0700`，
  前後被 revert 兩次。建立者可以決定自己建的目錄要什麼 mode，但不能去改一個本來就
  在那裡、而且不屬於我們的目錄。

  這個失效在結構上就是無聲的——目錄掉回 `0755` 不會報錯，每一筆查詢都照常跑，也不
  會出現在任何 diff 裡。只有直接對 mode 下斷言才抓得到，所以現在每個建立者都有一
  條。([#95](https://github.com/tznthou/ccRecall/issues/95))

- **Token 預算開始計算 hook 真正寫出去的東西** —— 「不超過 300 tokens」這個契約
  只算了 `content`，沒算 `- ` 前綴、confidence 後綴、`[key: …]` 把手，以及包在
  外面的 header 與 footer。2026-09-11 對真實 hook 實測（語料形狀的 payload）：
  endpoint 宣稱 225，hook 實際吐出 **363**。現在每行以渲染後的樣子計價、chrome
  先預留，`totalTokenEstimate` 回報的是整段輸出的大小而不只是那幾行。
  `DEFAULT_MAX_TOKENS` 由 300 提高到 400、對話中檢索的預算由 120 提高到 170，
  讓送到讀者眼前的記憶數量維持不變——**這不是多送內容，是讓契約不再是虛構的。**
  改動後端到端實測：契約 400、endpoint 宣稱 372、hook 吐出 366、一樣送 5 行。
- **對話中檢索不再因為單一記憶超出整個預算而整個沉默** —— 中文記憶大約每個字
  一個 token，在 149 字元上限下單獨一條就可能超出預算，呼叫端會收到空清單，
  與「沒有相關記憶」完全分不出來。現在第一行會被裁到放得下，而不是整條丟掉。
  預算依然不會被突破，而裁到只剩碎片、講不出東西的行則寧可不送。
- **SessionStart hook 不再自己寫死預算數字。** 它固定送 `maxTokens=300`，
  於是已安裝的舊 hook 會蓋掉後來升級過的服務。預算政策歸 daemon 管，
  跟對話中檢索的 hook 早就採用的做法一致。

### 變更

- **Tier 0 跨專案選擇改為 project-specific** — startup 用來浮現「其他專案」記憶
  的那三個席位，排序第一鍵是 `confidence DESC`，於是少數 `confidence = 1.0` 的
  記憶長期壟斷席位，1.0 以下的無論多相關都拿不到。三個排序鍵沒有一個是
  project-specific，所有專案其實在同一份全域排序上取前三：實測 80 個專案，整個
  語料庫只發得出 32 條不重複記憶，而且同一組三條在其中 15 個專案完全重複。

  排序改為 `相關性 band → 最久沒注入 → confidence`。band 用「這條記憶自己的
  topic 有多少比例落在當前專案」評分，是第一個會因專案而異的鍵。它刻意做成二元
  而非連續分數：分數若近乎每條唯一，rotation 只會在完全同分時才觸發，等於把
  confidence 壟斷換成相關性壟斷。另有 rank 保底，即使沒有任何記憶跨過比例門檻，
  仍讓比例最高的前 `TIER0_RELEVANCE_MIN_POOL` 條進 band。真正在選記憶的是保底不
  是門檻：80 個專案裡有 63 個，跨過比例門檻的記憶數少於保底名額，其中 52 個的候
  選集仍大於保底名額——那 52 個才是保底真的在挑人，剩下 11 個候選集本來就不足，
  保底等於整池放行。

  改動後同一組量測：整個語料庫發出 **74 條不重複記憶**（2.3 倍），最常重複的那組
  結果從 15 個專案降到 6 個——這 6 個是候選集為空的專案，真正共用同一組三條的最大
  一群只有 4 個。Tier 0 查詢延遲 3.9ms → 6.9ms（平均），最慢的單一專案約 22ms，
  對照的是 plan 給 Tier 0 的 30ms 門檻。

  band 用的是「匹配 topic 佔該記憶總 topic 的比例」而不是原始交集數：記憶的 topic
  數會跟著內容長度走（全語料庫 Spearman +0.78），直接用交集數就會重演 L1 那次的長
  度偏差。正規化擋掉了「長記憶靠 topic 數量取勝」，但沒有讓 band 對長度中立——除以
  一個長度代理變數只會把相關性翻向負的（逐專案中位數 −0.53），不會抵消掉。

- **抽取改成先寫處方，不先講故事** — 一條記憶送進 session 時只有前 149 個字元，
  而原本的寫法把可執行的那段擺最後：現象、根因，最後才是該怎麼做。實測 118 條帶
  明確處方標記的記憶，其中 116 條——**98.3%**——標記落在截斷線之後，中位位置 446
  字元，是截斷線的三倍。送到眼前的是一份被拿掉處方的現象報告，這解釋了為什麼召回
  看起來很健康、同樣的坑卻一直重踩。extraction prompt 現在要求把動作寫進第一句。

  驗收刻意沒有交給量測腳本，而這個決定是對的。腳本找的是 `Mitigation:` 這類標記，
  但一條「第一句就是處方」的記憶根本沒有理由再掛標記，於是在那把尺底下這個改動讀
  起來像退步：改動後可定位的 17 條裡 0 條落在線內，改動前是 54 條裡 1 條。同樣兩個
  窗口抽出 79 條做人工盲審——判準事先寫死，兩組混合打散、判完才揭曉——結果相反：
  **改動後 40 條中 28 條，改動前 39 條中 13 條**，Fisher exact p = 0.0016，而那 28
  條裡有 23 條全文根本沒有任何標記。仍有約三分之一的記憶開頭先鋪背景；這些逐條讀過
  全文，處方都寫了，只是落在截斷線之後。那三分之一是截斷線的問題，不是 prompt 的
  問題。

- **`scripts/l3-prescription-position.py` 改成量可操作性，不量有沒有標記** — 承上，
  標記定位器已經不是適合這批文字的儀器。新的定位器找的是「撐過截斷還讀得到的指令」，
  它的動詞表來自對語料自己做的頻率掃描，不是憑印象寫出來的。標記定位器保留為對照
  口徑並列印出；當兩者以上述方向矛盾時，腳本會直接指出哪個數字不該引用。附帶三個
  旗標：`--test`（合成案例，完全不碰資料庫）、`--self-check`（拿人工標註集替定位器
  打分，開發集與 holdout 分開報——因為定位器是看著第一批調的，recall 在那裡 0.75，
  在凍結後才標註的那批是 0.48）、`--until`（框住窗口上界；少了它，一次「改動前」的
  量測會把它自己要當對照組的那一段一起算進去）。後續審查在這份工作裡找到的九條缺陷
  記在 #117，刻意不在這一版修。
...
- **`recall_query` 的工具描述誤述了自己的範圍** — 原本寫「project-aware
  ranking」與涵蓋「across sessions and projects」，但實際上是 project 硬過濾：
  其他專案的記憶根本不在結果集裡。模型讀了舊描述就無從分辨「這裡沒有」和
  「被擋住了」，於是空結果被當成「這個知識不存在」的證據。新描述明講範圍，
  補上「沒人提起過去時也該查」的觸發條件（決定架構模式或引入依賴之前），
  並寫明查詢構造：詞給多不給少（索引是 trigram + BM25，靠匹配詞數區辨）、
  不要用 `AND`、把概念轉成英文（中文查詢零命中）、查模式的通用名稱而非本專案
  的專有名詞。依據是 180 組由「重複發現」記錄建成的跨專案配對；本機 recall
  telemetry 顯示真實 `recall_query` 呼叫有 66% 零命中。

### 修正

- **telemetry log 的保護有三個洞：會洩漏，也繞得過** — #89 帶進來的保護會把
  `extract.log.jsonl` 以 `0600` 建立、既有檔案也修回 `0600`。這次補掉其中三個洞。

  建檔那一步是唯一沒加 `2>/dev/null` 的。log 無法被保護時，shell 的重導錯誤會在
  stderr 印出完整路徑——正是那個權限要遮住的帳號名與專案名。靜默降級是這個函式的
  契約，要不要說話由呼叫端決定。

  一個別的本機使用者寫得進去的目錄，等於讓對方可以先在 log 的路徑上種一個
  symlink，接著 `chmod` 跟寫入都會跟著它走——這道保護會一邊回報成功，一邊改掉
  受害檔案的權限，再把使用者的 `cwd` append 進去。現在它會拒絕 symlink，也拒絕
  不是普通檔案的 log。但這只是**把窗口縮小**，不是關上：檢查、`chmod`、寫入是
  三次各自獨立的路徑解析，寫得進那個目錄的攻擊者仍然可以在它們之間種進去而贏得
  這場 race。要真的關上得用 `open(O_NOFOLLOW|O_APPEND)` 加 `fchmod` 在同一個
  descriptor 上完成，shell 寫不出來——真正的解法是那個目錄本來就不該讓別人寫。

  這個檔案建立出來的目錄現在一律是 `0700`，而且兩條路徑都是：extraction 那條會
  在 telemetry 寫入前約九十行就把目錄建好，而 `mkdir -p` 不會動已存在的目錄，
  所以只保護後面那一個，會讓同一個函式在提早 skip 時給出 `0700`、其他情況給出
  `0755`。

  已經存在的目錄則**不會**被改權限，所以在已有該目錄的安裝上，底下那層曝險仍然
  開著。`~/.ccrecall` 在 TypeScript 那側也會被建立，而且同樣沒指定 mode
  （`src/core/database.ts`、`src/core/integrity-monitor.ts`；只有
  `src/core/recall-telemetry.ts` 有傳 `0o700`），誰先跑到就由誰決定——通常是
  `0755`，在設了 `umask 002` 的機器上是 `0775`。兩種「修復既有目錄」的做法在
  review 過程中都實作過、也都退掉了。「改權限」會去動 ccRecall 並不擁有的目錄：
  `CCRECALL_EXTRACT_LOG` 設成相對路徑時，那個上層目錄就是當下的工作目錄，實測
  把一個專案目錄從 `0755` 改成了 `0700`。「拒絕使用 group 可寫的目錄」則會讓
  umask 為 `002` 的機器——那是好幾個發行版的 user-private-group 預設——每一筆
  telemetry 都被永久丟掉，而且不會有任何訊息。修既有目錄這件事屬於上面列的那些
  建立者，另案處理。

  兩個呼叫端都以 `|| :` 結尾。它們原本包在 `if` 條件裡，而 `errexit` 對 `if`
  條件是豁免的；裸命令沒有這個豁免，所以在 `set -e` 下，一筆被丟掉的 telemetry
  會取代掉 Claude 真正的 exit status——在開了 `errexit` 的互動 shell 裡，還會
  直接把使用者的 shell 關掉。

  兩個寫入點各自配一份保護、各自寫一次重導，等於「日後每個呼叫端都要記得」才安全。
  現在統一走 `_ccrecall_log_append`——權限屬於檔案，不屬於呼叫端：只要一個沒被保護的
  寫入在寬鬆 umask 下跑過，這個 log 就會以 `0644` 重生，其他寫入者全部繼承。

  這些現在都有測試釘住：權限退回 `0644` 不會報錯，在任何 telemetry 查詢裡也看不出
  差別。保護的三個步驟各自的失敗都是實際觸發出來測的，不是靠檢查程式碼長相帶過；
  「保持安靜」這條也在 zsh 下驗過——實際 source 這支 wrapper 的就是 zsh。

- **`cwd` → `project_id` 編碼漂移讓整個專案靜默**（#89）— Claude Code 在
  `~/.claude/projects/` 底下命名資料夾的規則，是把 `[A-Za-z0-9]` 以外的每個
  字元換成 `-`。ccRecall 只換了 `/`，所以工作目錄只要含空格、`.`、`_` 或中日韓
  文字，算出來的路徑就指向一個不存在的目錄。過程零報錯：`/session/last` 回
  404，而 404 跟「這個專案還沒有 session 被索引」長得一模一樣，於是 extraction
  wrapper 讀到空的 `sessionId`，以 `no-session-id` 略過——那個字串已經有四個
  彼此無關的成因會產生。受影響的專案會無限期地一則記憶都存不進去，但它的
  hooks 每次 prompt 都照常執行。

  拿一台 dogfood 機器上所有能取回 `cwd` 的專案目錄實測：char-wise 規則重現
  24/24，舊的只換斜線規則 21/24，漏掉的三個分別是空格、`.` 和中日韓文字。
  那台機器上有一個天天在用的專案，26 個已索引 session、連續 16 次 extraction
  略過、零記憶。

  有兩個副產品值得寫下來。第一，資料庫裡**沒有任何一筆**格式錯誤的
  `project_id`——那看起來像健康，實際上正好相反：錯誤的 key 根本寫不進來。
  第二，這個編碼規則是 Claude Code 的、不是我們的，所以現在改由回歸測試釘住
  （空格／中日韓／`.`／`_`），而且明確斷言 char-wise：byte-wise 的實作——BSD
  的 `sed` 和 `tr` 給你的就是那種——每個中日韓字元會吐出三個 dash。

  wrapper 不再自己推導。`/session/last` 直接回傳 indexer 從磁碟讀到的 id，
  wrapper 原樣使用；這是移掉一份 shell 端實作，而不是再寫一份正確的。
  `extract.log.jsonl` 的 skip 列現在多記 `cwd`——查詢沒落地時唯一還在的事實，
  這次能診斷出來就是靠它。該 log 現在一律以 `0600` 建立，既有檔案也會在每次
  附加前修正權限：它會揭露帳號名稱與使用者經手的每個專案名，而在寬鬆 umask
  下第一次被建立出來的 log 會是 `0644`。兩個寫入點都走同一道保護，因為權限
  屬於檔案而不屬於呼叫端——只保護其中一個等於兩個都沒保護，先跑到的那個決定
  了模式。權限無法確保時直接丟掉該筆紀錄：少一列 telemetry 比洩漏路徑便宜。

  第五個入口是 MCP 的 `projectId` 參數，那是模型自己推導的，改 code 碰不到。
  它的描述原本只示範了斜線分隔的路徑，模型照著做，遇到含空格的 cwd 一樣會
  算出錯的 id。現在描述講明字元規則，範例本身就帶一個空格。
