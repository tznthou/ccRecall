...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'ccrecall-db-'))
...
describe('indexSession', () => {
  it('writes project + session + message_uuids atomically', () => {
    db.indexSession({
      sessionId: 'sess-001',
      projectId: 'proj-1',
      ...
      messages: [
        msg({ type: 'user', role: 'user', uuid: 'u1', sequence: 0 }),
        msg({ type: 'assistant', role: 'assistant', uuid: 'a1', sequence: 1 }),
      ],
    })
  ...
  it('re-index replaces session row and cascades message_uuids', () => {
    db.indexSession({
      ...
      messages: [msg({ type: 'user', role: 'user', uuid: 'old-uuid', sequence: 0 })],
    })
    ...
    db.indexSession({
      ...
      startedAt: '2024-06-01T10:00:00.000Z', endedAt: '2024-06-01T10:00:00.000Z',
      messages: [msg({ type: 'user', role: 'user', uuid: 'new-uuid', sequence: 0 })],
    })
...
describe('archiveStaleSessionsExcept', () => {
  it('archives sessions not in keepIds set (session row + uuids preserved)', () => {
    db.indexSession({
    ...
    expect(sessions.find(s => s.id === 'keep-me')!.archived).toBe(false)
    expect(sessions.find(s => s.id === 'archive-me')!.archived).toBe(true)
  ...
  it('re-indexing an archived session un-archives it', () => {
    db.indexSession({
      ...
      fileMtime: '2024-01-01T00:00:00.000Z', startedAt: null, endedAt: null,
      messages: [msg({ type: 'user', role: 'user', uuid: 'rev-u1', sequence: 0 })],
    })
    ...
    db.indexSession({
      ...
      messages: [msg({ type: 'user', role: 'user', uuid: 'rev-u2', sequence: 0 })],
    })
