...
| `recall_save` | 儲存新記憶。選填 `key` slug 做 dedup（同 key 更新而非重複），並自動抽取 topics 供跨專案檢索。選填 `origin`（`explicit` \| `agent-inferred`）標記是誰在寫：`agent-inferred` 撞到既有的 `explicit` 記憶會被擋下，自動抽取蓋不掉你手寫的內容。選填 `messageId` 記下這筆記憶出自哪則訊息——會跟該 session 真正有的訊息核對，對不上就丟掉不存 |
