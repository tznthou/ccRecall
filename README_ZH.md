...
| `/metacognition/check?topic=...` | GET | 知識深度檢查 | Stub（Phase 3） |
| `/session/checkpoint` | POST | Pre-compact 存檔點 | Stub（Phase 3） |

## MCP 工具

| 工具 | 用途 |
|------|------|
| `recall_query` | 透過 FTS5 搜尋過去的決策、發現、pattern |
...
│   └── README.md              # Hook 安裝指南
├── tests/                     # 207 個測試橫跨 parser / scanner /
